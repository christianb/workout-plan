...
- **Biceps:** Pull day runs a single Zottman curl 3×10–12 (biceps supinated on the way up, brachioradialis pronated on the way down) — distinct from Arms day. Arms day order (Sep 2026): Seated dumbbell curl 3×6–8 (heavy bilateral, done first while freshest) → One-arm dumbbell preacher curl 3×10–12 (long-head stretch, done second while quality remains) → Dumbbell hammer curl 3×10–12 (brachialis finisher — neutral grip is more fatigue-tolerant than the supinated curls). Hammer curls are intentionally 2×/cycle (Legs light + Arms heavy), not triple-covered.
- **Arms-day biceps order (Sep 2026):** Preacher curl was moved from third to second (hammer curl moved to the finisher slot). User reported the preacher being very exhausting after the preceding curls, when it is one of the best long-head/stretch hypertrophy lifts — as the last movement it got the least load and worst execution. Hammer curl's neutral grip is more fatigue-tolerant, so it absorbs the finisher role without hurting the stimulus. Note: this reorder shifts `done[3]` ticks among the three biceps exercises.
- **Arms-day section order (Sep 2026):** The light Back block was moved from between Biceps and Triceps to after Triceps (new order: Biceps → Triceps → Back (light) → Forearms → Abs). It previously interrupted the two priority arm muscles with systemic and grip/forearm fatigue from rows and the straight-arm pullover right before the triceps growth block; the placement rationale on the lat pullover ("done before Triceps so triceps aren't the limiting factor") was also factually wrong — a straight-arm pullover does not involve the triceps. Back is maintenance, so it now sits after the priority work. **This swaps `done[3]` section keys 1 and 2** (Back ↔ Triceps) — expect stale ticks on both blocks.
...
- **Glutes (Sep 2026):** Hip thrust (machine) 3×10–12 added to Legs day, appended last in the `Quads & glutes` section (after leg extensions — append-only, so existing `done[4]` ticks are unaffected). The day's only prior hip-extension work was the Bulgarian split squat, which loads glutes at long/stretched lengths; the leg press barely extends the hip and prone curls are pure knee flexion — the thrust is the sole short-length/peak-contraction glute stimulus and lets glutes progress without adding split-squat sets (deliberately capped at 2/leg for time). Placed after both heavy compounds so it can't compromise the squat or press, yet early enough that systemic fatigue stays low for loading; the following prone curl is knee flexion, unaffected by fatigued glutes. Rest auto-derives to 75s from the 10–12 rep bucket; single machine, no superset. Takes the day back to 49 work sets (~2h → ~2h15min) — a deliberate exception to the trim above, accepted because this was the program's one genuine coverage gap.
